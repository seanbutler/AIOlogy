...
std::vector<double> ANN::load_image(const std::string& filename) {
    ...
    if (!sdl_manager.is_initialized()) {
        std::cerr << "SDL not initialized, falling back to simulation" << std::endl;
    ...
    // Fallback simulation (when SDL is not available)
    if (extension == ".png" && filename.find("mnist") != std::string::npos) {
        std::cout << "Simulating MNIST 28x28 grayscale image loading..." << std::endl;
        
        // Create a 28x28 = 784 pixel vector with simulated grayscale values
        std::vector<double> pixels;
        pixels.reserve(784);
        
        // Generate some realistic MNIST-like pixel values (0-255 range)
        // This simulates a digit with some pattern
        for (int y = 0; y < 28; ++y) {
            for (int x = 0; x < 28; ++x) {
                // Create a simple pattern that looks like it could be a digit
                double value = 0.0;
                
                // Add some variation based on position
                if (x > 5 && x < 22 && y > 5 && y < 22) {
                    // Inner area - simulate digit pixels
                    value = 128.0 + (x + y) % 127;
                } else {
                    // Border area - mostly background
                    value = (x * y) % 50;
                }
        ...
        std::cout << "Generated " << pixels.size() << " pixel values (simulated)" << std::endl;
        return pixels;
    }

    // For other files, return empty (not implemented yet)
    std::cout << "Image loading not yet implemented for: " << filename << std::endl;
    return {};
}
